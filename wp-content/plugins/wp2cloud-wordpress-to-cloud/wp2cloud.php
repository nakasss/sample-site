...
/*
Copyright (c) 2012-2014, OblakSoft LLC.
...
{
    //--------
    // Include optional configuration.  This configuration can be used
    // to set WP_LOAD_PATH without modifying the plugin sources.
    if(file_exists('wp2cloud-config.php'))
        include 'wp2cloud-config.php';

    //--------
    // When this file is referred directly, we need to know where to find
    // the WordPress engine.  By default, it's assumed that wp2cloud.php
    // is located in the
    // [ROOT]/wp-content/plugins/wp2cloud-wordpress-to-cloud/ directory.
    if(!defined('WP_LOAD_PATH')) {
        // Note: prior to 1.3.15 the user was supposed to set ABSPATH in
        // wp2cloud-config.php, so we still handle that.
        if(defined('ABSPATH'))
            define('WP_LOAD_PATH', ABSPATH);
        else
            define('WP_LOAD_PATH', dirname(dirname(dirname(dirname(__FILE__)))) . '/');
    }
...
require_once('wp2cloud-move.php');
...
define('CURRENT_SITE_URL', get_option('siteurl'));
...
class Wp2CloudPlugin {
    ...
    function ensure_cloud_table($die_on_error=true) {
        ...
        catch(Exception $e)
        {
            if($die_on_error)
            {
                $fmt =  __('Cannot create <b>%1$s</b> table -- %2$s  %3$s requires the Cloud Storage Engine for MySQL
 (%5$s) to be installed and configured on the MySQL server.  As an example of property configured
 %4$s and %3$s you can use %6$s.','wp2cloud');
                wp_die(sprintf($fmt, $this->ClouSE_table_name, $e->getMessage(), "WP2Cloud", "ClouSE",
                     "<a href='http://www.oblaksoft.com/documentation/getting-started-with-clouse/'>ClouSE</a>",
                     "<a href='http://www.slideshare.net/artemlivshits/wordpress-on-s3-stepbystep'>WordPress on S3</a> / " .
                     "<a href='http://www.slideshare.net/artemlivshits/wordpress-to-google-cloud'>WordPress to Google</a>"));
            }
        }
    }

    function activate() {
        load_plugin_textdomain('wp2cloud', false, dirname(plugin_basename( __FILE__ )) . '/lang' );
    ...
    function query_safe($db, $sql_query, $params=false)
    {
        if ($params)
        {
            foreach ($params as &$v) { $v = $db->_real_escape($v); }    // Escape the params
    ...
    function addhooks() {
        ...
        add_action('switch_blog', array(&$this, 'switch_blog'), 9, 2);
        ...
        add_filter('woocommerce_get_downloadable_file_urls', array(&$this, 'woocommerce_get_downloadable_file_urls'),9, 4);
        add_filter('woocommerce_file_download_paths', array(&$this, 'woocommerce_file_download_paths'),9, 4);
    ...
    //--------
    // Actions to be executed after plugin is loaded.
    function wp_loaded() {
        load_plugin_textdomain('wp2cloud', false, dirname(plugin_basename( __FILE__ )) . '/lang' );
    }

    //--------
    // Update table names and blog dir when blog is switched.
    function switch_blog($new_blog, $old_blog) {
        ...
        //--------
        // Check if we need to switch wp2cloud URL to represent the
        // switched-to site.  We use full URLs (with hostname) so that
        // domain-based multisite works.
        $site_url = get_option('siteurl');

        if(CURRENT_SITE_URL != $site_url)
        {
            if (!empty($current_blog) && strpos($this->wp2cloud_dir_url, $current_blog->path) === 0)
                $this->wp2cloud_dir_url = substr($this->wp2cloud_dir_url, strlen($current_blog->path));

            $this->wp2cloud_dir_url = rtrim($site_url, '/') . '/' . ltrim($this->wp2cloud_dir_url, '/');
        }
    }
    ...
    // Plugin settings.
    function settings() {
        $m2c_title = __('Move media to cloud', 'wp2cloud');
        $m2t_title = __('Move tables to cloud', 'wp2cloud');

        add_submenu_page(null, $m2c_title, $m2c_title, 'manage_options', 'wp2cloud-move-media', 'wp2cloud_render_media_move');
        add_submenu_page(null, $m2t_title, $m2t_title, 'install_plugins', 'wp2cloud-move-tables', 'wp2cloud_render_tables_move');

        $title = __('WordPress to Cloud', 'wp2cloud');
        add_options_page($title, $title, 'manage_options', 'wp2cloud-settings', 'wp2cloud_settings');
    }
    ...
    {
        $plugin_array['wp2cloud'] = $this->wp2cloud_dir_url . 'wp2cloud-mce.php';
        return $plugin_array;
    }

    //--------
    // Get ClouSE licensing information from the engine.
    function get_clouse_license()
    {
        $sql = "SELECT * FROM INFORMATION_SCHEMA.SESSION_STATUS WHERE VARIABLE_NAME LIKE 'clouse_%' " .
            "UNION SELECT * FROM INFORMATION_SCHEMA.GLOBAL_VARIABLES WHERE VARIABLE_NAME='clouse_version'";
        $r = $this->media_db->get_results($sql, ARRAY_N);
        ...
        foreach ($r as $record)
            $info[strtolower($record[0])] = $record[1];

        return $info;
    }

    //--------
    // Translate license SKU code to name.
    function get_license_sku_name($code)
    {
        $sku_codes = array('TRY' => 'Trial', 'BAS' => 'Basic', 'STD' => 'Standard', 'PRO' => 'Professional');
        return $sku_codes[$code];
    }

    //--------
    // Get localized licesing messages.
    function get_license_fmt_strings()
    {
        $expired_fmt = __('Your %1$s license expired on %2$s.', 'wp2cloud');
        $license_fmt = __('You have %1$s %2$s license valid until %3$s.','wp2cloud');
        $beta_fmt = __('You\'re running a pre-release version of %1$s that expires on %2$s. Please upgrade to the latest generally available version.', 'wp2cloud');
        return array($expired_fmt, $license_fmt, $beta_fmt);
    }
    ...
    {
        $lic = $this->get_clouse_license();

        //--------
        // Check license, but suppress any notices when installing
        // the license: it could be confusing to install the license
        // and see the old warning.
        if ($lic && empty($_POST['install_license']) && empty($_POST['paste_license']))
        {
            list($expired_fmt, $license_fmt, $beta_fmt) = $this->get_license_fmt_strings();

            if (isset($lic['clouse_license_exp']))
            {
                $expiration = $lic['clouse_license_exp'];
                $sku_name = $this->get_license_sku_name($lic['clouse_license_sku']);
                $now = time();

                if (strtotime($expiration) >= $now)
                {
                    //--------
                    // Warn if the license is trial or it expiress less than
                    // a month from now.
                    if ($lic['clouse_license_sku'] == 'TRY' ||
                        strtotime($expiration) < $now + 31 * 24 * 60 * 60)  // 31 days
                    {
                        $lic_message = sprintf($license_fmt, "<b>$sku_name</b>", "ClouSE", "<b>$expiration</b>");
                        echo '<div class="update-nag">'. $lic_message .'</div>';
                    }

                    if ($lic['clouse_license_err'])
                        echo '<div class="error"><p>ClouSE licensing error: ' . htmlspecialchars($lic['clouse_license_err']) . '</p></div>';
                }
                else
                {
                    $lic_message = sprintf($expired_fmt, "ClouSE", "<b>$expiration</b>");
                    echo '<div class="error"><p>'. $lic_message .'</p></div>';
                }
            }
            else
            {
                $lic_message = sprintf($beta_fmt, "ClouSE", "<b>2015-01-01</b>");
                echo '<div class="update-nag">'. $lic_message .'</div>';
            }
        }
        ...
        if((empty($options) || $options['ignore_nocloud'] != '1') &&
            $_GET['page'] != 'wp2cloud-move-tables')
        {
            //--------
            ...
            foreach($results as $r)
            {
                ...
                $tables .= '<strong>' . htmlspecialchars($r[0]) . '</strong>';
            }

            $extra = intval($found_rows[0][0]) - count($results);

            if($extra) {
                printf(_n('%1$s, and %2$d more table are not managed by the %3$s and not protected by the cloud storage.',
                          '%1$s, and %2$d more tables are not managed by the %3$s and not protected by the cloud storage.',
                          $extra, 'wp2cloud'),
                    $tables, $extra, 'Cloud Storage Engine');

            } else {
                printf(_n('%1$s table is not managed by the %2$s and not protected by the cloud storage.',
                          '%1$s tables are not managed by the %2$s and not protected by the cloud storage.',
                          count($results), 'wp2cloud'),
                    $tables, 'Cloud Storage Engine');
            }

            echo ' <a href="' . admin_url('options-general.php?page=wp2cloud-move-tables') . '" class="button">';
            echo __('Move to Cloud', 'wp2cloud');
            echo '</a></p></div>';
        }
    }
    ...
    function wp_handle_upload($data, $override)
    {
        ...
        if(isset($data['id']) && !file_exists($data['file']))
        {
            //--------
            // XMLRPC server calls this filter after the files are moved
            // to the cloud and doesn't actually pass valid file information
            // in the first place.  But it does pass attachment id, which
            // we use to get the data that we need.

            $file = get_attached_file($data['id'], true);

            if($file && strpos($file, '$wblob') !== false)
            {
                $data['url'] = $this->wp2cloud_dir_url.'wp2cloud.php?weblob_id=' . $this->weblob_id_from_path($file);
                return $data;
            }
        }
        ...
        if (file_exists($data['file'])) {
            ...
            $data['url'] = $this->wp2cloud_dir_url.'wp2cloud.php?weblob_id=' . $id_main;
    ...
    function wp_update_attachment_metadata($data, $postID) {
        ...
        if (strpos($main_file, '$wblob') === false)
        {
            //--------
        ...
        try
        {
            ...
            if(!isset($clouSE_upload_id[$data['file']]))
            {
                ...
                if ($this->move_function)
                    call_user_func($this->move_function, $main_file, $data['file']);
            }
            ...
            if (isset($data['thumb'])) {
                ...
                if ($this->move_function)
                    call_user_func($this->move_function, $thumbpath, $thumbid);
            }
            ...
            if (count($data['sizes'])) {
                foreach ($data['sizes'] as $altName => $altSize) {
                    ...
                    if ($this->move_function)
                        call_user_func($this->move_function, $altPath, $altId);
                }
            }
        ...
        {
            //--------
            // Update attached file after uploading the data.
            update_attached_file($postID, $data['file']);
        }

        //--------
        // Remove the local files no matter what so the user will realize
        // something went wrong right away, except for during move, as
        // the deletion will happen as a separate phase.
        if (!$this->move_function)
    ...
    function wp_get_attachment_url($url, $postID) {
        ...
        if (strpos($_SERVER['PHP_SELF'], '/wp-admin/') === false && !defined('WP_ADMIN'))
        {
            return $weblob_url;
        }
        ...
        return $this->wp2cloud_dir_url . 'wp2cloud.php?weblob_id=' . $weblob_id;
    }
    ...
    function xlate_path_url($matches)
    {
        ...
        return $weblob_url ? '"' . $this->wp2cloud_dir_url . 'wp2cloud.php?weblob_id=' . $matches[1] .'"' : $matches[0];
    }
    ...
    //--------
    // Translate URLs in WooCommerce downloadable products.
    function woocommerce_get_downloadable_file_urls($file_urls, $product_id, $variation_id, $item)
    {
        $result_urls = array();

        foreach ($file_urls as $url => $download) {
            $result_urls[$this->wbid_url_to_cloud($url)] = $download;
        }
    ...
    //--------
    // Translate URLs in WooCommerce downloadable products.
    function woocommerce_file_download_paths($file_paths, $product_id, $order_id, $item)
    {
        $result_paths = array();

        foreach ($file_paths as $download_id => $path) {
            $result_paths[$download_id] = $this->wbid_url_to_cloud($path);
        }
    ...
    function wp2cloud_get_file_url($file)
    {
        ...
        //-------
        // The file is not in the cloud, but it could be on the server
        // (e.g. because it hasn't yet been uploaded to the cloud yet).
        $uploads = wp_upload_dir();

        if ($uploads['error'] !== false)
            return $file;

        $weblob_path = $uploads['basedir'] . '/$wblob/' . $weblob_id;

        if (file_exists($weblob_path) && is_file($weblob_path)) {
            return $uploads['baseurl'] . '/$wblob/' . $weblob_id;
        }
    ...
    //--------
    // For multisite, get blog id for a given URL.
    function get_blog_id_from_url($url)
    {
        $parts = parse_url($url);
        $domain = $parts['host'];

        // This logic is copied from ms-settings.php  due to lack of a
        // reusable function.
        ...
        if ( is_subdomain_install() ) {
            ...
            $path = preg_replace( '|([a-z0-9-]+.php.*)|', '', $parts['path'] );
            $path = str_replace ( '/wp-admin/', '/', $path );
            $path = preg_replace( '|(/[a-z0-9-]+?/).*|', '$1', $path );

            $blogname = htmlspecialchars( substr( $parts['path'], strlen( $path ) ) );
        ...
        return $current_blog ? $current_blog->blog_id : false;
    }
    ...
    function xlate_wbid_url($matches)
    {
        ...
        // $matches[2] - the plugin URL, e.g. /wp-contents/plugins/wp2cloud-wordpress-to-cloud
        // $matches[3] - the weblob_id, e.g. foo/42
        // $matches[4] - single or double quote, or paren

        $switched_blog = false;

        if (is_multisite() && $matches[2][1] != '/')
        {
            // In multisite, the URL could point to a different site in the
            // same network, in which case we need to switch to the context
            // of the other site.
            $switched_blog = $this->get_blog_id_from_url($matches[2]);

            if ($switched_blog !== false)
                switch_to_blog($switched_blog);
        }

        $weblob_url = $this->get_url_from_weblob_id($matches[3]);

        if ($switched_blog !== false)
            restore_current_blog();

        return $weblob_url ? $matches[1] . $weblob_url . $matches[4] : $matches[0];
    }
